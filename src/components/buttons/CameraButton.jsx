export default function CameraButton({ disabled, onClick, children, complete }) {
  return (
    <div className="calibration-camera-button">
      <button
        className={`calibration-click ${complete ? "is-complete" : "calibration-click--accuracy"}`}
        disabled={disabled}
        onClick={onClick}
        type="button"
      >
        <svg
          className="calibration-camera-button__icon"
          width="24"
          height="24"
          viewBox="0 0 24 24"
          fill="none"
      ...
      <span className="calibration-camera-button__label">{children}</span>
    </div>
  );
}
